...
import { useListAllProjects, useUpdateProject, useDeleteProject, getListAllProjectsQueryKey, getListProjectsQueryKey } from "@workspace/api-client-react";
    ...
    updateProject.mutate(
      { id, data: { published: !currentStatus } },
      {
        onSuccess: () => {
          ...
          toast({ title: `Project ${!currentStatus ? 'published' : 'unpublished'}` });
        }
      }
    );
  };
              ...
              <tbody className="divide-y divide-border">
                {projects.map((project) => (
                  <tr key={project.id} className="hover:bg-muted/20">
                    ...
                    <td className="p-4 hidden md:table-cell">{project.client}</td>
                    <td className="p-4 hidden lg:table-cell text-muted-foreground">{format(new Date(project.createdAt), 'MMM dd, yyyy')}</td>
                    <td className="p-4">
                      <Button 
                        variant="ghost" 
                        size="sm" 
                        className={`h-7 text-xs rounded-none ${project.published ? 'text-green-600 hover:text-green-700' : 'text-muted-foreground hover:text-primary'}`}
                        onClick={() => togglePublish(project.id, project.published)}
                      >
                        {project.published ? <Eye className="w-3 h-3 mr-2" /> : <EyeOff className="w-3 h-3 mr-2" />}
